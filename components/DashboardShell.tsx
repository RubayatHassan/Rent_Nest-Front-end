...
export function DashboardShell({ role, title, children }: { role: Role; title: string; children: ReactNode }) {
  ...
  useEffect(() => {
    if (role === "LANDLORD") {
      getLandlordRequests().then((requests) => setPendingRequests(requests.filter((request) => request.status === "PENDING").length)).catch(() => setPendingRequests(0));
      return;
    }
    if (role === "TENANT") {
      getMyRentals().then((rentals) => setPendingPayments(rentals.filter((rental) => rental.status === "APPROVED" && rental.payment?.status !== "COMPLETED").length)).catch(() => setPendingPayments(0));
    }
  }, [role, pathname]);
