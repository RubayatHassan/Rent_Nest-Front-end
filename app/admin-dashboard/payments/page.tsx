...
import { getAdminPayments, Payment } from "../../../lib/api";
...
export default function Page() {
  ...
  useEffect(() => {
    getAdminPayments("limit=100").then((result) => setItems(result.data || [])).catch((err) => setError(err instanceof Error ? err.message : "Unable to load payments"));
    const params = new URLSearchParams(window.location.search);
    setReturnStatus(params.get("payment") || params.get("status") || "");
  }, []);

  const filtered = useMemo(() => items.filter((payment) => `${payment.rentalRequest?.property?.title || ""} ${payment.rentalRequest?.tenant?.name || ""} ${payment.provider} ${payment.method || ""} ${payment.status} ${payment.transactionId || ""}`.toLowerCase().includes(query.toLowerCase())), [items, query]);
