...
export default function Page() {
  ...
  const loadPayments = () =>
    getAdminPayments("limit=100").then((result) => setItems(result.data || []));
  ...
  const filtered = useMemo(
    () =>
      items.filter((payment) =>
        `${payment.rentalRequest?.property?.title || ""} ${payment.rentalRequest?.tenant?.name || ""} ${payment.rentalRequest?.property?.landlord?.name || ""} ${payment.rentalRequest?.property?.landlord?.email || ""} ${payment.provider} ${payment.method || ""} ${payment.status} ${payment.transactionId || ""}`
          .toLowerCase()
  ...
  return (
    <div>
      ...
      {returnStatus && (
        <div className={`payment-return-banner ${returnStatus.toLowerCase()}`}>
          <strong>Gateway return: {returnStatus.toUpperCase()}</strong>
          <span>Payment result received from the success/cancel callback.</span>
        </div>
      )}
        ...
        <div className="table-tools">
          <div className="search-box">
            ...
            <input
              ...
              placeholder="Search payment, tenant, landlord or status..."
            />
          </div>
        ...
        >
          ...
          >
            <div className="drawer-head">
              <div>
                <p className="eyebrow">Admin payment details</p>
                <h2 id="admin-payment-details-title">
            ...
            <div className="payment-detail-grid">
              ...
              <div>
                <span>Landlord</span>
                <strong>
                  {selected.rentalRequest?.property?.landlord?.name ||
                    "Not available"}
                </strong>
              </div>
              <div>
                ...
                <strong>
                  {selected.rentalRequest?.property?.landlord?.email ||
                    "Not available"}
                </strong>
              </div>
            </div>
            ...
            {selected.gatewayResponse && (
              <div className="payment-gateway-response">
                <span>Gateway response</span>
                <pre>{JSON.stringify(selected.gatewayResponse, null, 2)}</pre>
              </div>
            )}
            ...
            {selected.status === "PENDING" && (
              <button
                ...
                onClick={() => setConfirmingPayment(selected)}
              >
      ...
      {confirmingPayment && (
        <div
          ...
          role="presentation"
          onClick={(event) => {
            if (event.target === event.currentTarget && !cancellingId)
              setConfirmingPayment(null);
          }}
        >
          <div
          ...
          >
            ...
            <h2 id="cancel-payment-title">Cancel payment?</h2>
            <p>
              This pending payment for{" "}
              <strong>
                {confirmingPayment.rentalRequest?.property?.title ||
                  "this rental"}
              </strong>{" "}
              will be marked as failed. This action cannot be undone.
            </p>
